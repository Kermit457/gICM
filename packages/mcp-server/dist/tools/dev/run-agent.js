...
const INTEGRATION_HUB_URL = process.env.GICM_INTEGRATION_HUB_URL || "http://localhost:3001";
export async function runAgent(agentId, input, options = {}) {
    const startTime = Date.now();
    try {
        // Call Integration Hub to execute agent
        const response = await axios.post(`${INTEGRATION_HUB_URL}/api/agents/execute`, {
            agentId,
            input,
            options: {
        ...
        if (axios.isAxiosError(error) && error.code === "ECONNREFUSED") {
            return {
                success: false,
                agentId,
                error: "Integration Hub not available. Start it with: pnpm --filter @gicm/integration-hub start",
                executionTime,
                autonomyDecision: {
                    action: "reject",
