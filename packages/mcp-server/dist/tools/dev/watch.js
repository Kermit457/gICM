 ...
 */
export async function watchStatus(projectPath) {
 ...
 */
export async function watchChanges(projectPath, limit = 20) {
    const targetPath = projectPath || process.cwd();
 ...
 */
export async function watchReindex(files, projectPath, contextEngineUrl) {
    const targetPath = projectPath || process.cwd();
 ...
 */
export async function watchClear(projectPath) {
    const targetPath = projectPath || process.cwd();
    const changesLogPath = path.join(targetPath, ".gicm", "changes.log");
    try {
        if (fs.existsSync(changesLogPath)) {
...
export const watchTools = {
    "dev.watch_status": {
        description: "Check if the file watcher is running and get statistics about file changes.",
        parameters: {
            project_path: {
                type: "string",
    ...
    "dev.watch_changes": {
        description: "Get recent file changes tracked by the watcher. Useful for seeing what files have been modified recently.",
        parameters: {
            project_path: {
                type: "string",
                description: "Path to project (defaults to current directory)",
    ...
    "dev.watch_reindex": {
        description: "Manually trigger reindexing of specific files. Use this when you want to ensure certain files are freshly indexed for semantic search.",
        parameters: {
            files: {
                type: "array",
    ...
    "dev.watch_clear": {
        description: "Clear the file changes log. Use this to start fresh with change tracking.",
        parameters: {
            project_path: {
                type: "string",
                description: "Path to project (defaults to current directory)",
