 ...
 */
function getProjectContext(): { type: string; language: string; frameworks: string[] } {
...
async function searchRelevantTools(
  task: string,
  context: { type: string; language: string; frameworks: string[] },
  limit: number
  ...
  try {
    ...
    if (response.data?.tools) {
      return response.data.tools.map((tool: Record<string, unknown>) => ({
...
function generateLocalSuggestions(
  ...
  const capabilityMap: Record<string, { id: string; name: string; kind: "agent" | "skill" | "command" | "mcp"; description: string }> = {
    // Trading/DeFi
 ...
 */
async function installCapabilities(ids: string[]): Promise<void> {
 ...
 */
function generateMessage(suggestions: CapabilitySuggestion[], autoInstalled: boolean): string {
