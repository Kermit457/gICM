...
// Use the actual type from @gicm/memory
...
export async function memoryRemember(params: {
  ...
  try {
    ...
    return { success: true, entry: entry as MemoryEntry };
  } catch (error) {
    return {
      success: false,
...
export async function memoryRecall(params: {
  key: string;
  namespace?: string;
}): Promise<{ found: boolean; entry?: MemoryEntry; error?: string }> {
  try {
    const { Memory } = await import("@gicm/memory");
    const mem = new Memory();
    ...
    return { found: !!entry, entry: entry as MemoryEntry | undefined };
  } catch (error) {
    return {
      found: false,
      error: error instanceof Error ? error.message : "Unknown error",
...
export async function memorySearch(params: {
  ...
  try {
    ...
    return { entries: entries as MemoryEntry[] };
  } catch (error) {
    return {
      entries: [],
      error: error instanceof Error ? error.message : "Unknown error",
...
export const memoryTools = {
  "dev.memory_remember": {
    description:
      "Store a memory for later recall. Use this to remember facts, preferences, decisions, or context that should persist across sessions.",
    parameters: {
      key: {
        type: "string",
      ...
      type: {
        type: "string",
        description:
          "Memory type: fact (immutable truth), preference (user choice), context (session info), decision (choice made), outcome (result)",
  ...
  "dev.memory_recall": {
    description:
      "Recall a stored memory by its key. Returns the memory if found and not expired.",
    parameters: {
      key: {
        type: "string",
  ...
  "dev.memory_search": {
    description:
      "Search memories by type, tags, or key pattern. Useful for finding related memories.",
    parameters: {
      type: {
        type: "string",
  ...
  "dev.memory_forget": {
    description:
      "Delete a memory. Use this when information is no longer valid or needed.",
    parameters: {
      key: {
        type: "string",
  ...
  "dev.memory_stats": {
    description:
      "Get memory statistics including total entries, types breakdown, and storage usage.",
    parameters: {},
  },
};
