 ...
 * gICM Autonomy CLI
 *
 * Commands for managing autonomous decision-making.
 */

import { AutonomySystem } from "./index.js";
...
function rejectAction(id: string, reason?: string): void {
...
function printAudit(sinceHours?: number): void {
  const since = sinceHours ? Date.now() - sinceHours * 60 * 60 * 1000 : undefined;
...
async function sendDaily(): Promise<void> {
...
async function main(): Promise<void> {
  ...
  switch (command) {
    ...
    case "reject":
      ...
      rejectAction(args[1], args.slice(2).join(" ") || undefined);
      break;
    ...
    case "audit":
      printAudit(args[1] ? parseInt(args[1], 10) : undefined);
      break;
