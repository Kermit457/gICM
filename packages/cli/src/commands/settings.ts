...
function validateSettingValue(settingId: string, value: any): { valid: boolean; error?: string; parsedValue?: any } {
...
// Add setting
async function addSetting(settingId: string, options: { value?: any; yes?: boolean }) {
  const spinner = ora();
    ...
    if (value === undefined) {
      if (!options.yes) {
        ...
        const response = await prompts({
          ...
          choices: 'enum' in setting && setting.enum ? setting.enum.map((v: any) => ({ title: v, value: v })) : undefined,
        });
...
// Remove setting
async function removeSetting(settingId: string, options: { yes?: boolean }) {
  const spinner = ora();
...
// List settings
async function listSettings(options: { category?: string; all?: boolean }) {
