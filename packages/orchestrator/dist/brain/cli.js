...
async function main() {
  ...
  switch (command) {
    ...
    case "phase":
      ...
      await runPhase(phase, args.includes("--live"));
      break;
    case "goals":
      showGoals();
      break;
...
async function startBrain(live) {
  console.log("\n=== gICM BRAIN ===\n");
...
async function sendHeartbeat(hubUrl) {
  try {
...
async function unregisterFromHub(hubUrl) {
  try {
...
async function runPhase(phase, live) {
  console.log(`
...
`);
  ...
  console.log(`
=== Phase Complete ===`);
  console.log(`Duration: ${((result.completedAt - result.startedAt) / 1e3).toFixed(1)}s`);
