...
program.name("wins").description("gICM Win Tracker - Track wins, streaks, achievements").version("1.0.0");
program.command("record").description("Record a win").requiredOption("-c, --category <category>", "Category: money|growth|product|intelligence|agent").requiredOption("-t, --title <title>", "Win title").requiredOption("-v, --value <value>", "Win value", parseFloat).option("-u, --unit <unit>", "Value unit", "count").option("-s, --subcategory <sub>", "Subcategory", "general").option("-d, --description <desc>", "Description").action(async (opts) => {
...
program.command("today").description("Show today's wins").action(async () => {
...
program.command("stats").description("Show all-time stats").action(async () => {
...
program.command("streak").description("Show streak info").action(async () => {
  const streak = await tracker.getStreak();
  console.log(chalk.bold.yellow(`
  STREAK INFO
`));
...
program.command("achievements").description("Show achievements").action(async () => {
