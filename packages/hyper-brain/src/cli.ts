#!/usr/bin/env node
/**
 * gICM HYPER BRAIN CLI
 */

import { Command } from "commander";
import chalk from "chalk";
import ora from "ora";
import { HyperBrain } from "./index.js";
import { BrainApiServer } from "./api/index.js";

const program = new Command();

const LOGO = `
${chalk.cyan("╔═══════════════════════════════════════════════════════════════╗")}
${chalk.cyan("║")}  ${chalk.yellow("🧠")} ${chalk.bold.white("gICM HYPER BRAIN")}                                          ${chalk.cyan("║")}
...
program
  .name("hyper-brain")
  .description("gICM HYPER BRAIN - Knowledge & Learning System")
  .version("1.0.0");

program
  .command("start")
  .description("Start the HYPER BRAIN")
  .option("-p, --port <port>", "API port", "3300")
  ...
  .action(async (options) => {
    ...
    let apiServer: BrainApiServer | null = null;
    ...
    try {
      ...
      console.log();
      console.log(chalk.green("  📡 Knowledge ingestion started"));
      console.log(chalk.green("  🔄 Processing pipeline active"));
      console.log(chalk.green("  🧠 Learning system enabled"));
      console.log(chalk.green("  🔮 Prediction engine ready"));
      if (options.api !== false) {
        console.log(chalk.green(`  🌐 API server: http://localhost:${options.port}`));
        if (options.ws !== false) {
          console.log(chalk.green(`  🔌 WebSocket: ws://localhost:${options.port}/ws`));
        }
      }
      console.log();
...
program
  .command("ingest")
  .description("Force ingest from all sources")
  .option("-s, --source <source>", "Specific source to ingest")
  .action(async (options) => {
    console.log(LOGO);
...
program
  ...
  .option("--analyze", "Run pattern analysis first")
  .action(async (options) => {
    const brain = new HyperBrain();
    await brain.start();
...
program
  .command("predict <type>")
  .description("Generate predictions (market|content|product|agent)")
  .option("-n, --count <count>", "Number of predictions", "5")
  .action(async (type, options) => {
    ...
    const predictions = await brain.predict(type as any, parseInt(options.count));
...
program
  .command("stats")
  .description("Show HYPER BRAIN statistics")
  .action(async () => {
    console.log(LOGO);
    ...
    console.log(chalk.bold("📊 HYPER BRAIN STATS\n"));
...
program
  ...
  .option("-n, --limit <limit>", "Number of items", "20")
  .action(async (options) => {
    const brain = new HyperBrain();
    await brain.start();
...
program
  .command("topics")
  .description("Show topic distribution")
  .action(async () => {
    const brain = new HyperBrain();
    await brain.start();
    ...
    // Count topics
    for (const item of brain.getRecent(1000)) {
