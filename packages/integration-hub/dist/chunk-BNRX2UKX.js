...
async function registerRoutes(fastify, hub) {
  ...
  fastify.post("/api/engines/register", async (req) => {
    ...
    const validEngines = ["brain", "money", "growth", "product", "trading", "opus67"];
...
import { AuthMiddleware, hasScope } from "@gicm/agent-core/security/auth";
...
async function authPlugin(fastify, options) {
  ...
  fastify.delete("/api/auth/keys/:keyId", async (request, reply) => {
    if (!request.user?.scope?.includes("admin") && !request.user?.scope?.includes("*")) {
      reply.code(403).send({
        ok: false,
        error: "Admin access required",
  ...
  fastify.decorate("requireScope", function(requiredScope) {
    return async (request, reply) => {
...
var auth_plugin_default = fp(authPlugin, {
  fastify: "5.x",
  name: "gicm-auth"
});
...
var ApiServer = class {
  ...
  async initialize() {
    ...
    await this.fastify.register(auth_plugin_default, {
      jwtSecret: process.env.GICM_JWT_SECRET,
      apiKeyPrefix: process.env.GICM_API_KEY_PREFIX || "gicm",
      enabled: process.env.NODE_ENV === "production" || process.env.GICM_AUTH_ENABLED === "true",
      publicRoutes: [
...
//# sourceMappingURL=chunk-BNRX2UKX.js.map