 ...
 * - boundaries: Show/update boundaries
...
import ora from "ora";
import { getAutonomy, resetAutonomy, type AutonomyEngine } from "./index.js";

const program = new Command();

program
  .name("gicm-autonomy")
  .description("Level 2+ Autonomy system for gICM platform")
  .version("1.0.0");

// Status command
program
  .command("status")
  .description("Show autonomy engine status")
  .action(() => {
...
program
  ...
  .option("-r, --reason <text>", "Rejection reason", "Rejected via CLI")
  .action(async (id, options) => {
...
program
  ...
  .description("Approve multiple requests")
  .option("--safe", "Approve all safe/low risk items")
  .option("--category <cat>", "Approve by category")
  .option("--risk <level>", "Approve by risk level")
...
program
  .command("boundaries")
  .description("Show or update operational boundaries")
  .option("--set <key=value>", "Set a boundary value")
  .option("--json", "Output as JSON")
  .action((options) => {
    const autonomy = getAutonomy();
...
program
  ...
  .action(() => {
    ...
    console.log(`  Oldest Age: ${Math.round(queueStats.oldestAge / 60000)} minutes`);
    console.log(`  Avg Wait: ${Math.round(queueStats.avgWaitTime / 60000)} minutes`);
